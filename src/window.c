...
static const char *enabled_validation_layers[] = {"VK_LAYER_KHRONOS_validation"};
...
static const u32 num_device_extensions = 1;
...
void window_free(Window *window) {
    ...
    vkDestroyDevice(window->logical_device.vk_handle, NULL);
    if (enable_val_layers) {
        vkDestroyDebugUtilsMessengerEXT(window->instance, window->debug_messenger, NULL);
    }
...
static void init_vulkan(Arena *arena, Window *window) {
    ...
    if (enable_val_layers) {
        result = vkCreateDebugUtilsMessengerEXT(window->instance, &debug_create_info, NULL,
    ...
    window->surface = create_surface(window->instance, window->handle);
    window->physical_device = pick_physical_device(scratch.arena, window->instance);
    window->logical_device =
        create_logical_device(scratch.arena, window->physical_device, window->surface);
