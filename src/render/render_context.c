...
void rnd_context_init(RND_Context *rc, Window *window) {
  ...
  rc->uploader = rnd_uploader_create(rc);
...
translation_local void create_logical_device(RND_Context *rc) {
  ...
  rc->uploader.transfer_index = family_indices.transfer;
  ...
  if (rc->graphic_index != rc->uploader.transfer_index) {
...
translation_local void create_swap_chain(RND_Context *rc, Window *window) {
  ...
  rc->swap.clear_color = (VkClearColorValue){.float32 = {0.2f, 0.3f, 0.3f, 1.0f}};
...
translation_local void create_frame_resources(RND_Context *rc) {
  ...
  for (u32 i = 0; i < rc->swap.frames_in_flight; i++) {
    VK_CHECK_FATAL(
        vkCreateSemaphore(rc->logical, &sem_info, NULL, &rc->swap.frames[i].image_available_sem),
        EXT_VK_SYNC_OBJECT, "Failed to create image available semaphore");
    LOG_DEBUG("Created image available semaphore %u", i);

    VK_CHECK_FATAL(
        vkCreateSemaphore(rc->logical, &sem_info, NULL, &rc->swap.frames[i].render_finished_sem),
